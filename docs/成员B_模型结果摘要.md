...
|------|--------|-----------|------------|-----------|
| 基准模型 | 12 | 0.2753 | 2.5053天 | 1.9005天 |
| 改进模型 | 20 | **0.3075** | **2.4491天** | **1.8590天** |

**模型改进**：
- R²提升：0.0321（从27.5%提升到30.8%）
- RMSE降低：0.0562天
- MAE降低：0.0415天

### 1.2 模型诊断结果

- **残差分析**：Shapiro-Wilk检验统计量=0.9383，p<0.001（样本量大时常见）
- **多重共线性**：所有变量VIF < 10，无严重多重共线性问题
- **模型解释力**：能解释约31%的住院天数方差
...
|------|--------|--------|--------|--------|------|-----|
| 基准模型 | 11 | 0.6844 | 0.1652 | 0.4519 | 0.2419 | 0.6179 |
| 改进模型 | 19 | **0.6597** | **0.1626** | **0.4947** | **0.2447** | **0.6150** |

**模型特点**：
- 使用类别权重处理数据不平衡（再入院率11.16%）
- 召回率49.5%，能识别约一半的再入院患者
- AUC=0.62，模型具有一定的区分能力
...
| 变量 | OR值 | 解释 |
|------|------|------|
| 既往住院次数（`number_inpatient`） | **1.47** | 既往住院次数每增加1次，再入院风险增加47% |
| 诊断数量（`number_diagnoses`） | **1.13** | 诊断数量每增加1个，再入院风险增加13% |
| 既往急诊次数（`number_emergency`） | **1.08** | 既往急诊次数每增加1次，再入院风险增加8% |
| 用药数量（`num_medications`） | **1.09** | 用药数量每增加1个，再入院风险增加9% |
| 出院去向分组（`discharge_disposition_group`） | **1.13** | 出院去向不同，再入院风险有差异 |
...
1. **既往住院次数是最强的风险因素**（OR=1.47）
2. **合并症数量（诊断数量）是重要风险因素**（OR=1.13）
...
- ✅ 使用类别权重处理不平衡问题
- ✅ 召回率49.5%，能识别约一半的再入院患者
- ⚠️ AUC=0.62，仍有改进空间（但考虑到数据不平衡和问题难度，可接受）
...
- `models/regression_model.pkl` - 回归模型（包含基准和改进模型）
- `models/classification_model.pkl` - 分类模型（包含基准和改进模型）

---

## 六、系统功能

原型系统（Streamlit）包含以下功能：
