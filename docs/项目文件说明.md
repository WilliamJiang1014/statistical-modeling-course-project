...
6. **06_feature_engineering.py** - 特征工程与数据分割
   - 输出：`data/processed/diabetic_data_featured.csv`、`data/processed/data_splits.pkl`

7. **07_regression_model.py** - 回归模型（住院天数预测）
   - 输出：`models/regression_model.pkl`、`docs/figures/07-08_*.png`（2张图表）

8. **08_classification_model.py** - 分类模型（30天再入院预测）
   - 输出：`models/classification_model.pkl`、`docs/figures/09-12_*.png`（4张图表）

9. **09_streamlit_app.py** - Streamlit交互式Web系统
   - 运行方式：`streamlit run code/09_streamlit_app.py`
...
- **data_splits.pkl** - 训练/测试集分割结果（80/20）
...
- **regression_model.pkl** - 回归模型文件
  - 模型类型：多元线性回归
  - 用途：预测住院天数
  - 生成脚本：`code/07_regression_model.py`
- **classification_model.pkl** - 分类模型文件
  - 模型类型：Logistic回归
  - 用途：预测30天再入院风险
  - 生成脚本：`code/08_classification_model.py`
...
2. **依赖安装**：运行前请先安装依赖：`pip install -r requirements.txt`
